...
export default function WikiEditor({ apiId, initialContent = '', onSave }: WikiEditorProps) {
  ...
  const insertMarkdown = (syntax: string, placeholder: string = '') => {
    ...
    const textarea = document.querySelector(
      'textarea[data-wiki-editor="true"]'
      ...
      <AnimatePresence mode="wait">
        {editing ? (
          ...
          >
            ...
            <div className="flex border-b" style={{ borderColor: 'rgba(0, 0, 0, 0.05)' }}>
              <button
                ...
                style={{
                  ...
                  backgroundColor:
                    activeTab === 'edit' ? 'rgba(var(--primary-blue-rgb), 0.05)' : 'transparent',
                }}
              >
                <svg
                  width="16"
                ...
                style={{
                  ...
                  borderBottom:
                    activeTab === 'preview'
                      ...
                      : '2px solid transparent',
                  backgroundColor:
                    activeTab === 'preview' ? 'rgba(var(--primary-blue-rgb), 0.05)' : 'transparent',
                }}
              >
                <svg
                  width="16"
            ...
            {activeTab === 'edit' && (
              <div
                className="flex flex-wrap gap-1 p-3 border-b bg-gray-50"
                style={{ borderColor: 'rgba(0, 0, 0, 0.05)' }}
              >
                <ToolbarButton onClick={() => insertMarkdown('h1')} title="제목 1">
                  H1
                </ToolbarButton>
                <ToolbarButton onClick={() => insertMarkdown('h2')} title="제목 2">
                  H2
                </ToolbarButton>
                <ToolbarButton onClick={() => insertMarkdown('h3')} title="제목 3">
                  H3
                </ToolbarButton>
                <div className="w-px h-6 bg-gray-300 mx-1" />
                <ToolbarButton onClick={() => insertMarkdown('bold')} title="굵게">
                  <strong>B</strong>
                </ToolbarButton>
                <ToolbarButton onClick={() => insertMarkdown('italic')} title="기울임">
                  <em>I</em>
                </ToolbarButton>
                <ToolbarButton
                  ...
                  {text ? (
                    <ReactMarkdown
                      ...
                      components={{
                        h1: ({ node: _node, ...props }) => (
                          <h1
                            className="text-[32px] font-bold mb-4 mt-6 break-words"
                            style={{ color: 'var(--text-dark)' }}
                            {...props}
                          />
                        ),
                        h2: ({ node: _node, ...props }) => (
                          <h2
                            className="text-[28px] font-bold mb-3 mt-5 break-words"
                            style={{ color: 'var(--text-dark)' }}
                            {...props}
                          />
                        ),
                        h3: ({ node: _node, ...props }) => (
                          <h3
                            className="text-[24px] font-bold mb-2 mt-4 break-words"
                            style={{ color: 'var(--text-dark)' }}
                            {...props}
                          />
                        ),
                        p: ({ node: _node, ...props }) => (
                          <p
                            className="mb-4 text-[16px] break-words"
                            style={{ color: 'var(--text-dark)' }}
                            {...props}
                          />
                        ),
                        ...
                        code: ({ node: _node, inline, ...props }: any) =>
                          inline ? (
                            <code
                              className="px-2 py-1 rounded text-[14px] font-mono break-all"
                              style={{
                            ...
                            <code
                              className="block p-4 rounded-[12px] text-[14px] font-mono overflow-x-auto"
                              style={{
                                backgroundColor: 'rgba(0, 0, 0, 0.05)',
                                maxWidth: '100%',
                                whiteSpace: 'pre-wrap',
      ...
      {editing && activeTab === 'edit' && (
        ...
        >
          <div className="flex items-start gap-2">
            ...
            <div>
              <strong style={{ color: 'var(--primary-blue)' }}>Markdown 팁:</strong> 굵게:
              **텍스트** | 기울임: *텍스트* | 링크: [텍스트](url) | 코드: `code` | 제목: # H1, ##
              H2, ### H3
            </div>
          </div>
        </motion.div>
