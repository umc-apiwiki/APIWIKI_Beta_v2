    ...
    >
      <div className="relative p-5 h-full flex flex-col">
        ...
        <div className="flex gap-4 mb-3">
          {/* Logo Image */}
          <div className="w-12 h-12 min-w-[3rem] rounded-[8px] flex-shrink-0 bg-white flex items-center justify-center overflow-hidden shadow-image p-2">
            {api.logo ? (
              api.logo.length > 4 ||
          ...
          <div className="flex flex-col gap-0.5 min-w-0 flex-1 justify-center">
            {/* Title */}
            <div className="text-gray-900 text-sm font-bold font-sans truncate leading-tight">
              {api.name}
            </div>
            ...
            <div className="flex flex-col gap-0">
              ...
              <div className="text-slate-600 text-[0.6875rem] font-medium font-sans flex items-center gap-0.5">
                {api.rating ? <span className="text-amber-400 text-[0.7rem]">★</span> : null}
        ...
        {/* Description */}
        <div
          className={`text-slate-600 text-[0.6875rem] font-normal font-sans line-clamp-2 leading-relaxed h-[1.75rem] ${hideCompare ? 'mb-4' : 'mb-8'}`}
        >
          {api.description}
        </div>
        ...
        {!hideCompare && (
          ...
          >
            <div className="w-24 h-7 relative group cursor-pointer">
              <div
                className={`w-24 h-7 absolute rounded-full transition-all shadow-button border-sky-500 border ${isCompareSelected ? 'bg-sky-500' : 'bg-white group-hover:bg-sky-50'}`}
              />
              <div
                className={`w-full h-full absolute flex items-center justify-center text-xs font-semibold font-sans leading-none ${isCompareSelected ? 'text-white' : 'text-sky-500'}`}
              >
