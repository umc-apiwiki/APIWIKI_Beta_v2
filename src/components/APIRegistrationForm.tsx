...
export default function APIRegistrationForm({ onSubmit, onCancel }: APIRegistrationFormProps) {
  ...
  const handlePricingChange = (tier: 'free' | 'basic' | 'pro' | 'csv', value: string) => {
    setFormData((prev) => {
      const safePricing =
        typeof prev.pricing === 'object' && prev.pricing !== null ? prev.pricing : {};
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            onChange={handleChange}
            className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent"
            placeholder="예: https://example.com/logo.png 또는 🗺️"
          />
        </div>
      ...
      <div className="space-y-3">
        ...
        <p className="text-xs text-gray-500">
          각 플랜을 쉼표(,)로 구분하여 입력하세요. 예: 무료 플랜 설명, 기본 플랜 설명, 프로 플랜
          설명
        </p>
        ...
        <div>
          ...
          <textarea
            id="pricing-csv"
            value={
              (typeof formData.pricing === 'object' && formData.pricing !== null
                ? formData.pricing.csv
                : '') || ''
            }
      ...
      {/* 주요 기능 */}
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">주요 기능 (선택사항)</label>
