  ...
  const handleCheckName = async () => {
    ...
    try {
      const response = await fetch(
        `/api/user/check-name?name=${encodeURIComponent(formData.name)}`,
        {
          method: 'GET',
          credentials: 'include',
        }
  ...
  return (
    ...
    >
      ...
      <div
        className={
          isMobile
            ? 'px-4 pt-16 pb-20 space-y-6 relative z-10'
            : 'max-w-6xl mx-auto px-6 pt-28 pb-20 space-y-6 relative z-10'
        }
      >
            ...
            <div className="space-y-3">
              {message && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  className="p-3 rounded-xl bg-emerald-50 border border-emerald-100 text-emerald-800 text-sm"
                >
                  {message}
                </motion.div>
              )}
              {error && (
                <motion.div
                  initial={{ opacity: 0, y: -6 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="p-3 rounded-xl bg-rose-50 border border-rose-100 text-rose-700 text-sm"
                >
                  {error}
                </motion.div>
              )}
